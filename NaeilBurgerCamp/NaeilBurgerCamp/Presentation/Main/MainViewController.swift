...
final class MainViewController: UIViewController {
    ...
    private lazy var collectionView: UICollectionView = {
        ...
        collectionView.register(MenuItemFooterView.self, forSupplementaryViewOfKind: UICollectionView.elementKindSectionFooter, withReuseIdentifier: MenuItemFooterView.identifier)
        collectionView.register(CartSectionHeaderView.self, forSupplementaryViewOfKind: UICollectionView.elementKindSectionHeader, withReuseIdentifier: CartSectionHeaderView.identifier)
    ...
    func collectionView(_ collectionView: UICollectionView, viewForSupplementaryElementOfKind kind: String, at indexPath: IndexPath) -> UICollectionReusableView {
        if kind == UICollectionView.elementKindSectionFooter && indexPath.section == 1 {
            let footerView = collectionView.dequeueReusableSupplementaryView(
                ofKind: kind,
                withReuseIdentifier: MenuItemFooterView.identifier,
                for: indexPath
            ) as! MenuItemFooterView
            ...
            let headerView = collectionView.dequeueReusableSupplementaryView(
                ofKind: kind,
                withReuseIdentifier: CartSectionHeaderView.identifier,
                for: indexPath
            ) as! CartSectionHeaderView
    ...
    private func updateCurrentPageFromScrollPosition() {
        ...
        if let firstVisibleItem = layoutAttributes.sorted(by: { $0.indexPath.item < $1.indexPath.item }).first {
            ...
            if newPage != currentPage {
                ...
                // 푸터 버튼 상태 업데이트
                if let footerView = collectionView.visibleSupplementaryViews(ofKind: UICollectionView.elementKindSectionFooter).first as? MenuItemFooterView {
...
extension MainViewController: MenuItemFooterViewDelegate {
    ...
    private func updateFooterButtonState(footerView: MenuItemFooterView) {
        let maxPage = (totalMenuItems + itemsPerPage - 1) / itemsPerPage - 1
